...
![SigLIP2架构](/image/Siglip2.png){.center style="width:85%;"}


SigLip2在视觉上做了很多工作，首先将多个任务加入训练过程，保留sigmoid loss，并加入LocCa (基于标题的定位)，强制模型通过写标题（Captioning）来关注物体的具体位置，同时，在训练的后20%，加入SILC (自蒸馏)，通过“局部看整体”的训练，让模型具备了强大的语义联想能力；TIPS (掩码预测)，通过“特征补全”的完形填空，让模型对每一个像素块（Patch）都有深刻的理解。四种任务使SigLIP 2 不仅能做分类，还能胜任分割、深度估计等需要“像素级理解”的稠密任务

另外，NaFlex的提出是对ViT的一个重要改进，过去我们要么忍受图片变形，要么浪费计算量。而这里的NaFlex融合两种技术，NaViT保持原始宽高比。图片是长是圆，我就按原样切块（Patches），lexiViT支持变长序列。模型不仅能处理 256 个 token，也能处理 128 个或者 1024 个。
也就是说，实现动态宽高比以及序列长度的灵活切换，简单任务用短序列省算力，复杂任务用长序列看细节。 极大地提升了模型在现实世界复杂场景（非正方形图）下的鲁棒性和推理效率.

在数据侧，还提出 ACID 隐式蒸馏，它不强迫学生模仿老师的输出，而是让老师当“审稿人”。利用大模型（So400m）对数据进行实时打分和筛选，只选出对学生模型有价值的数据，实现了“低成本、高效率”的知识传递。让 ViT-B 这种中型模型也能通过 40 亿条精选数据，达到甚至超过部分大模型的性能。
