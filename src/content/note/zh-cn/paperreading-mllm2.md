...
，在视觉与语言对齐上的Visual Resampler，使用的是corss-attn，和Flamingo 模型中的 Perceiver Resampler原理差不多，都是用一个可以学习的query与image_token产生的kv做cross-attn，这种做法对全局的学习能力更强，并且相比于llava中的mlp有一个好处，就是无论token多大，经过attn后，传入llm的都是query长度的token，也就意味着对高分辨率的图片天然很友好，虽然后续llava的工作也在对高分辨率图像上做了很多split and encode到的改进，但是思路和这篇文章是不一样的。
